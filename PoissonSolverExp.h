...
protected:
  ...
  void makeFlux(const cubism::BlockInfo &rhs_info, const int ix, const int iy,
                const int iz, const bool &isBoundary,
                const cubism::BlockInfo &rhsNei, const FaceCellIndexer &indexer,
                SpRowInfo &row) const;
  ...
  public:
    ...
    long long This(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz) const {
    ...
    long long Xmax(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz, const int offset = 0) const {
      return blockOffset(info) +
             (long long)(iz * ny_ * nx_ + iy * nx_ + (nx_ - 1 - offset));
    }
    long long Xmin(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz, const int offset = 0) const {
      return blockOffset(info) +
             (long long)(iz * ny_ * nx_ + iy * nx_ + offset);
    }
    long long Ymax(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz, const int offset = 0) const {
      return blockOffset(info) +
             (long long)(iz * ny_ * nx_ + (ny_ - 1 - offset) * nx_ + ix);
    }
    long long Ymin(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz, const int offset = 0) const {
      return blockOffset(info) +
             (long long)(iz * ny_ * nx_ + offset * nx_ + ix);
    }
    long long Zmax(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz, const int offset = 0) const {
      return blockOffset(info) +
             (long long)((nz_ - 1 - offset) * ny_ * nx_ + iy * nx_ + ix);
    }
    long long Zmin(const cubism::BlockInfo &info, const int ix, const int iy,
                   const int iz, const int offset = 0) const {
      return blockOffset(info) +
             (long long)(offset * ny_ * nx_ + iy * nx_ + ix);
    }

  protected:
    long long blockOffset(const cubism::BlockInfo &info) const {
      return (info.blockID +
              ps.Nblocks_xcumsum_[ps.sim.lhs->Tree(info).rank()]) *
             nxyz_;
    }
  ...
  /*
    Class to help with indexing of neighbours for cell located at a face of a
    block. To provide a generic API for flux constructors at block faces, static
    polymorphism determines how indexing will be carried out with methods
    overloaded in [XYZ]{max,min}Indexer.

      - neiUnif gives the index of a cell across the face assuming the
    neighbouring block is at the same refinement level

    Define [XYZ]{max,min}Indexer using an inheritence model which needs 10 class
    definitions instead of 6, but avoids two-fold reimplementation of some
    methods
  */
  ...
  public:
    ...
    virtual long long neiUnif(const cubism::BlockInfo &nei_info, const int ix,
                              const int iy, const int iz) const = 0;
    ...
    // Order of neiFine{1,2,3,4} must be consistent with [sign_ds1, sign_ds2] of
    // corresponding dimension and have order [-1,-1], [1,-1], [-1,1], [1,1]
    ...
    virtual long long neiFine2(const cubism::BlockInfo &nei_info, const int ix,
                               const int iy, const int iz,
                               const int offset = 0) const = 0;
    virtual long long neiFine3(const cubism::BlockInfo &nei_info, const int ix,
                               const int iy, const int iz,
                               const int offset = 0) const = 0;
    virtual long long neiFine4(const cubism::BlockInfo &nei_info, const int ix,
                               const int iy, const int iz,
                               const int offset = 0) const = 0;

    // Indexing aids for derivatives in taylor approximation, info, ix, iy, iz
    // of a coarse cell
    virtual double taylorSign(const int ix, const int iy, const int iz,
                              const int dimShift) const = 0;
    virtual bool isBD(const int ix, const int iy, const int iz,
                      const int dimShift) const = 0;
    virtual bool isFD(const int ix, const int iy, const int iz,
                      const int dimShift) const = 0;
    virtual long long Nei(const cubism::BlockInfo &info, const int ix,
                          const int iy, const int iz, const int dist1,
                          const int dist2) const = 0;
  ...
  public:
    ...
    double taylorSign(const int ix, const int iy, const int iz,
                      const int dimShift) const override {
      return dimShift == 1 ? (iy % 2 == 0 ? -1. : 1.)
                           : (iz % 2 == 0 ? -1. : 1.);
    }
    bool isBD(const int ix, const int iy, const int iz,
              const int dimShift) const override {
      return dimShift == 1 ? (iy == ny_ - 1 || iy == ny_ / 2 - 1)
                           : (iz == nz_ - 1 || iz == nz_ / 2 - 1);
    }
    bool isFD(const int ix, const int iy, const int iz,
              const int dimShift) const override {
      return dimShift == 1 ? (iy == 0 || iy == ny_ / 2)
                           : (iz == 0 || iz == nz_ / 2);
    }
    long long Nei(const cubism::BlockInfo &info, const int ix, const int iy,
                  const int iz, const int dist1,
                  const int dist2) const override {
  ...
  public:
    ...
    long long neiUnif(const cubism::BlockInfo &nei_info, const int ix,
                      const int iy, const int iz) const override {
    ...
    long long neiFine3(const cubism::BlockInfo &nei_info, const int ix,
                       const int iy, const int iz,
                       const int offset = 0) const override {
      return Xmax(nei_info, ix_f(ix), iy_f(iy), iz_f(iz) + 1, offset);
    }
    long long neiFine4(const cubism::BlockInfo &nei_info, const int ix,
                       const int iy, const int iz,
                       const int offset = 0) const override {
  ...
  public:
    ...
    double taylorSign(const int ix, const int iy, const int iz,
                      const int dimShift) const override {
      return dimShift == 1 ? (iz % 2 == 0 ? -1. : 1.)
                           : (ix % 2 == 0 ? -1. : 1.);
    }
    bool isBD(const int ix, const int iy, const int iz,
              const int dimShift) const override {
      return dimShift == 1 ? (iz == nz_ - 1 || iz == nz_ / 2 - 1)
                           : (ix == nx_ - 1 || ix == nx_ / 2 - 1);
    }
    bool isFD(const int ix, const int iy, const int iz,
              const int dimShift) const override {
      return dimShift == 1 ? (iz == 0 || iz == nz_ / 2)
                           : (ix == 0 || ix == nx_ / 2);
    }
    long long Nei(const cubism::BlockInfo &info, const int ix, const int iy,
                  const int iz, const int dist1,
  ...
  public:
    ...
    double taylorSign(const int ix, const int iy, const int iz,
                      const int dimShift) const override {
      return dimShift == 1 ? (ix % 2 == 0 ? -1. : 1.)
                           : (iy % 2 == 0 ? -1. : 1.);
    }
    bool isBD(const int ix, const int iy, const int iz,
              const int dimShift) const override {
      return dimShift == 1 ? (ix == nx_ - 1 || ix == nx_ / 2 - 1)
                           : (iy == ny_ - 1 || iy == ny_ / 2 - 1);
    }
    bool isFD(const int ix, const int iy, const int iz,
              const int dimShift) const override {
      return dimShift == 1 ? (ix == 0 || ix == nx_ / 2)
                           : (iy == 0 || iy == ny_ / 2);
    }
    long long Nei(const cubism::BlockInfo &info, const int ix, const int iy,
                  const int iz, const int dist1,
  ...
  // Array of the indexers above pointed to by a base pointer for polymorphism
  // in makeFlux
  std::array<const FaceCellIndexer *, 6> faceIndexers;

  std::array<std::pair<long long, double>, 3>
  D1(const cubism::BlockInfo &info, const FaceCellIndexer &indexer,
     const int ix, const int iy, const int iz, const int dimShift) const {
    bool ds1 = dimShift == 1;
  ...
  std::array<std::pair<long long, double>, 3>
  D2(const cubism::BlockInfo &info, const FaceCellIndexer &indexer,
     const int ix, const int iy, const int iz, const int dimShift) const {
    bool ds1 = dimShift == 1;
    std::array<int, 2> b2{ds1 ? -2 : 0, ds1 ? 0 : -2};
    std::array<int, 2> b1{ds1 ? -1 : 0, ds1 ? 0 : -1};
  ...
  std::array<std::pair<long long, double>, 4>
  Dmixed(const cubism::BlockInfo &info, const FaceCellIndexer &indexer,
  ...
  void interpolate(const cubism::BlockInfo &info_c, const int ix_c,
                   const int iy_c, const int iz_c,
                   const cubism::BlockInfo &info_f,
                   const long long fine_close_idx, const long long fine_far_idx,
                   const double sign, const double sign_ds1,
                   const double sign_ds2, const FaceCellIndexer &indexer,
                   SpRowInfo &row) const;
};
